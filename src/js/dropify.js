...
export class Dropify {
  ...
  createElements() {
    ...
    if (this.isDisabled && this.settings.showRemove) {
      this.clearButton = getElementFromString(this.settings.tpl.clearButton);
  ...
  readFile(input) {
    if (input.files && input.files[0]) {
      ...
      if (
        ...
        this.element.addEventListener("dropify.fileReady", this.onFileReady);
  ...
  setPreview(previewable, src) {
    ...
    this.filenameWrapper.querySelector(".dropify-filename-inner").innerHTML =
      this.file.name;
  ...
  showError(errorKey) {
    if (typeof this.errorsContainer !== "undefined") {
      this.errorsContainer
        .querySelector("ul")
        .append(
          getElementFromString("<li>" + this.getError(errorKey) + "</li>"),
        );
    }
  }
